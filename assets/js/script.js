...
/**
 * Funções de Roteamento e Histórico
 */
function updateURL(params) {
  const url = new URL(window.location);
  // Limpa parâmetros antigos relevantes
...
function handlePopState(event) {
  ...
  if (article && module) {
    renderArticle(module, article, false); // false = não atualizar URL
  } else if (module) {
    renderTopics(module, false);
  } else {
    renderCategories(false);
  }
}

function getLinkHref(moduleId, articleId) {
...
function handleLinkClick(e, callback) {
  e.preventDefault();
  callback();
}
...
function renderCategories(updateHistory = true) {
  ...
  app.innerHTML = `
    ...
    <div class="module-grid">
      ${forumData.map(category => `
        <a href="${getLinkHref(category.id)}" onclick="handleLinkClick(event, () => renderTopics('${category.id}'))" class="card-link flex cursor-pointer flex-col gap-4 rounded-xl p-5 group">
...
function renderTopics(categoryId, updateHistory = true) {
  const category = forumData.find(c => c.id === categoryId);
  if (!category) return renderCategories();

  state.currentCategory = categoryId;
  if (updateHistory) updateURL({ module: categoryId });
  ...
  const topicsHTML = Object.entries(groups).map(([groupName, topics]) => `
    <div class="article-group" id="group-${groupName.replace(/\s+/g, '-').toLowerCase()}">
      ...
      <div class="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4">
        ${topics.map(topic => `
          <a href="${getLinkHref(category.id, topic.id)}" onclick="handleLinkClick(event, () => renderArticle('${category.id}', '${topic.id}'))" class="card-link flex cursor-pointer flex-col gap-3 rounded-xl p-5 group">
  ...
  const groupLinksHTML = groupNames.map(name =>
    `<a onclick="scrollToGroup('${name.replace(/\s+/g, '-').toLowerCase()}'); this.closest('.group-filter-menu').classList.remove('visible');" 
       class="group-filter-item cursor-pointer">${name}</a>`
  ).join('');
...
function renderArticle(categoryId, topicId, updateHistory = true) {
  ...
  state.currentCategory = categoryId;
  if (updateHistory) updateURL({ module: categoryId, article: topicId });
  ...
  app.innerHTML = `
    <div class="flex flex-wrap items-center gap-2 text-sm font-medium">
      <a href="/" onclick="handleLinkClick(event, () => renderCategories())" class="cursor-pointer opacity-70 hover:opacity-100">Início</a>
      <span class="opacity-50">/</span>
      <a href="${getLinkHref(category.id)}" onclick="handleLinkClick(event, () => renderTopics('${category.id}'))" class="cursor-pointer opacity-70 hover:opacity-100">${category.title}</a>
      <span class="opacity-50">/</span>
...
function renderSidebar() {
  sidebarNav.innerHTML = forumData.map(category => `
    <a href="${getLinkHref(category.id)}" onclick="handleLinkClick(event, () => renderTopics('${category.id}'))" 
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Route Handling on Load
  const params = new URLSearchParams(window.location.search);
  const loadingModule = params.get('module');
  const loadingArticle = params.get('article');

  if (loadingArticle && loadingModule) {
    renderArticle(loadingModule, loadingArticle, false);
  } else if (loadingModule) {
    renderTopics(loadingModule, false);
  } else {
    renderCategories(false);
  }
